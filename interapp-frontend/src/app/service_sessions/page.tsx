...
const handleFetchServiceSessionsData = async (
  ...
  if (res.status !== 200) throw new ClientError({ message: 'Failed to fetch service sessions', responseStatus: res.status, responseBody: res.data});
  // then we get the services for searching
  const res2 = await apiClient.get('/service/all');
  if (res2.status !== 200) throw new ClientError({ message: 'Failed to fetch services', responseStatus: res2.status, responseBody: res2.data });
